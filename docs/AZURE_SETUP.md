...
## 9. Assigning Groups to Entra ID Roles

The Project Setup Tool supports assigning created security groups to Entra ID roles, enabling role-based access control for project resources.

### Required Permissions for Role Assignment

In addition to the group management permissions, the Processing app needs permission to assign groups to directory roles:
...
6. Search for and select **RoleManagement.ReadWrite.Directory**
7. Click **Add permissions**
...
# Add the RoleManagement.ReadWrite.Directory application permission
# The permission ID for RoleManagement.ReadWrite.Directory is 9e3f62cf-ca93-4989-b6ce-bf83c28f9fe8
az ad app permission add \
  --id $PROCESSING_APP_ID \
  --api 00000003-0000-0000-c000-000000000000 \
...
Project security groups can be assigned to Entra ID roles through the API:

#### Using the API

```bash
# Assign a group to a role
POST /api/project/{projectId}/group/roles/{roleId}

# Remove a group from a role
DELETE /api/project/{projectId}/group/roles/{roleId}
```

#### Role Assignment Storage

Role assignments are stored in the `GroupInfo` object in the `AssignedRoleIds` property. This information is:
- Persisted in Azure Table Storage as part of the project's group information
- Displayed in the Permissions tab of the project page
- Included in the project API responses

### Common Entra ID Roles

Some commonly used Entra ID built-in roles include:
- **Global Administrator** - Full access to all administrative features
- **Application Administrator** - Can manage applications and service principals
- **Cloud Application Administrator** - Can manage cloud applications
- **User Administrator** - Can manage users and groups
- **Helpdesk Administrator** - Can reset passwords for non-administrators

To get the role IDs, use the Microsoft Graph API:
...
- **Principle of Least Privilege**: Only assign groups to roles that are necessary for the project's requirements
- **Audit Role Assignments**: Regularly review which groups are assigned to which roles
- **Admin Consent Required**: The `RoleManagement.ReadWrite.Directory` permission requires admin consent
- **Monitor Usage**: Use Azure AD audit logs to track role assignment changes

### Troubleshooting

**Error: "Insufficient privileges to complete the operation" when assigning roles**
- Ensure the Processing app has `RoleManagement.ReadWrite.Directory` permission
- Verify admin consent has been granted for this permission

**Role assignments not showing in UI**
- Check that the `AssignedRoleIds` property is properly serialized in the database
- Verify the API is correctly returning the group information with role assignments
...
- [Microsoft Graph Groups API](https://learn.microsoft.com/en/graph/api/resources/group)
- [Microsoft Graph Directory Roles API](https://learn.microsoft.com/en-us/graph/api/resources/directoryrole)
